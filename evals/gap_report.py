...
(extracted from `docs/VCRM.md`), and emits `VCRM_GAPS_<run_id>.md` beside the
...
# --------------------------------------------------------------------------
# BR catalogue. Lifted from docs/VCRM.md; keep these in sync if docs/VCRM.md is edited.
...
BR_CATALOGUE: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "covering_evals": {},
        "baseline": "full",
        "notes": "Verified by Inspection (docs/ARCHITECTURE.md + folder layout)",
...
def generate_markdown(summary: Dict[str, Any]) -> str:
    ...
    lines.append("Reflects this run's outcomes only. For the static catalogue see `docs/VCRM.md` and `docs/VCRM_GAPS.md`.")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("---")
    lines.append("")
    lines.append("Companion: `docs/VCRM.md` (catalogue), `docs/VCRM_GAPS.md` (static gap analysis), `docs/TEST_CONDITIONS.md` (every test).")
