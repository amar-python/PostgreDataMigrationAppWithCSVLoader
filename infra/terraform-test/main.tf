# main.tf - Azure Test environment for PostgreDataMigrationApp
#
# Same architecture tier as Prod (infra/terraform-prod/main.tf) — like
# Staging, Test deliberately does NOT reuse Dev's cheaper Container Apps
# Postgres, so automated runs against Test exercise the same managed-service
# behavior (private networking, managed Flexible Server) that Prod has:
...
# VNet CIDR (10.42.0.0/20 vs Prod's 10.40.0.0/20, Staging's 10.41.0.0/20) so
# none ever collide if peered, and defaults sized down where the variable
# descriptions call it out (see variables.tf) — override via terraform.tfvars.
#
...
terraform {
  ...
  # Test state MUST be in remote storage, same as Prod. Uncomment after
  ...
  # substituting "test" for "prod" (bucket name, key, etc.).
  #
  # backend "azurerm" {
  #   resource_group_name  = "rg-tfstate"
  #   storage_account_name = "sttfstatetestXXXXXX"  # from the one-time setup script
...
provider "azurerm" {
  features {
    key_vault {
      purge_soft_delete_on_destroy    = false # Test: never purge by accident
...
resource "azurerm_virtual_network" "vnet" {
  name                = "vnet-te-test-${local.suffix}"
...
resource "azurerm_private_dns_zone" "pg" {
  name                = "te-test-${local.suffix}.private.postgres.database.azure.com"
  resource_group_name = azurerm_resource_group.rg.name
  tags                = local.tags
}
...
resource "azurerm_postgresql_flexible_server" "pg" {
  name                          = "psql-te-test-${local.suffix}"
  resource_group_name           = azurerm_resource_group.rg.name
  location                      = azurerm_resource_group.rg.location
...
resource "azurerm_postgresql_flexible_server_database" "te_test" {
  name      = "te_test"
...
resource "azurerm_user_assigned_identity" "runner" {
  name                = "id-runner-test-${local.suffix}"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
  tags                = local.tags
}
...
resource "azurerm_container_app_job" "migration" {
  ...
  template {
    container {
      name   = "migration"
      image  = "${azurerm_container_registry.acr.login_server}/te-migration:test-initial"
      cpu    = 1.0
      memory = "2Gi"
      ...
      env {
        name  = "PGDATABASE"
        value = azurerm_postgresql_flexible_server_database.te_test.name
      }
      env {
        name        = "PGPASSWORD"
